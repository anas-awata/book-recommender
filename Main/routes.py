...
from Main.recomm import recom, bookdisp, recom_by_favorites,recommend_books
from Main.models import Favorite, User, WantToRead, ReadBefore, Clicked
...
def get_book_by_name(book_name):
    ...
    if not book.empty:
        # Get the first matching book's details
        book_info = book.iloc[0].to_dict()
        index = book_info.get('index')

        # Find the image URL corresponding to the book's index
        image = images_df[images_df['index'] == index]
...
@login_required
def favorites():
    recommended_books = recom_by_favorites(current_user.id)
